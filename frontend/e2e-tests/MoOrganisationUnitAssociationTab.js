...
// Manager
const searchAssociationEmployee = dialog.find('.v-autocomplete[data-vv-as="Medarbejder"]')
const searchAssociationItem = searchAssociationEmployee.find('.v-autocomplete-list-item')
...
test('Workflow: organisation association tab empty association', async t => {
  ...
  await t
    .click(VueSelector('organisation-detail-tabs bTabButtonHelper').withText('Tilknytninger'))
    // Create association
    ...
    .expect(searchAssociationItem.withText(' ').visible).ok()
    .pressKey('down enter')
    .expect(searchAssociationEmployee.find('input').value).match(/Jens/)
