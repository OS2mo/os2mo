...
located in `oio_rest`{.interpreted-text role="file"}. You can run it in
...
TL;DR: To get a running development environment with postgres and mox,
run:
...
The repository contains a `Dockerfile`{.interpreted-text role="file"}.
...
`Database`{.interpreted-text role="ref"} for a complete description.

Remember if you set the `setting <settings>`{.interpreted-text
role="ref"} `[database] host = "localhost"` the container will try to
connect to a database in the same container, not the host machine.

You will need to `initialize the database, user and
extensions<db_user_ext_init>`{.interpreted-text role="ref"}. The docker
...
objects<db_object_init>`{.interpreted-text role="ref"} for on the first
startup.
...
extensions<db_user_ext_init>`{.interpreted-text role="ref"}, you can
...
For other setting you can set, see `settings`{.interpreted-text
role="ref"}.
...
on how to run test see `Testing`{.interpreted-text role="ref"} and
`Docker-compose-testing`{.interpreted-text role="ref"}.
...
`/var/mox`{.interpreted-text role="file"} is the default
`[file_upload] folder <settings>`{.interpreted-text role="ref"} value.
...
same `UID/GID` as the
`user that runs the application <Docker-user>`{.interpreted-text
role="ref"} in the `Dockerfile`{.interpreted-text role="file"}.

User permissions
----------------

::: {#Docker-user}
The `Dockerfile`{.interpreted-text role="file"} creates and runs the
...
If you change the `UID/GID`, the `/var/mox`{.interpreted-text
role="file"} volume may not have the right permissions. It is
...
A `docker-compose.yml`{.interpreted-text role="file"} for development is
...
`oio_rest`{.interpreted-text role="file"} and the server will be
...
name of the service in `docker-compose.yml`{.interpreted-text
role="file"}. The `--build` flag builds the newest docker image for
`oio_rest` from the local `Dockerfile`{.interpreted-text role="file"}.
...
The `docker-compose.yml`{.interpreted-text role="file"} file contains a
service named `mox-cp`. Its purpose is to copy the files needed to
`initialize the database, user and
extensions<db_user_ext_init>`{.interpreted-text role="ref"} to a volume.
...
Testing in docker-compose {#Docker-compose-testing}
-------------------------
...
described in `Testing`{.interpreted-text role="ref"}. pgTAP is installed
in the database via the
`dev-environment/postgres.Dockerfile`{.interpreted-text role="file"} and
the required database user privilegedes are granted via
`dev-environment/postgres-initdb.d/30-upgrade-to-superuser.sh`{.interpreted-text
role="file"}
...
`oio_rest`{.interpreted-text role="file"}.

Prerequisites
-------------

The `oio_rest`{.interpreted-text role="file"} package requires a few
...
-   `git` for installing some requirements from
    `requirements.txt`{.interpreted-text role="file"} and
-   `libxmlsec1-dev` for the Python package `xmlsec`.

Mox needs to connect to `postgres11.7`. mox can be configured with the
`setting <settings>`{.interpreted-text role="ref"}
`[database] host = "localhost"` to connect to any machine. The default
value of `[database] host <settings>`{.interpreted-text role="ref"} is
`"localhost"`.
...
Look through the `settings`{.interpreted-text role="ref"} and configure
...
::: {.todo}
Refer to relavant section in `database`{.interpreted-text role="ref"}
