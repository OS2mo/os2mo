...
MO has a powerful topic-based event system. Combined with the GraphQL API, this allows you to build integrations between OS2mo and any other system where changes take an almost immediate effect. Additionally, OS2mo's event system can be used by integrations themselves to emit events.

The entire event system is exposed through [MO's GraphQL
interface](./graphql/intro.md). GraphQL is self-documenting which means a lot of the documentation can be found in the schema, for example by visiting `/graphql` in a browser.

The event system guarantees at-least-once delivery. There might be multiple events emitted even if there are no observable changes. As such, event handlers in integrations SHOULD be idempotent.
...
A namespace is a logical group for listeners. A namespace will often correspond to a single application. You should only create a namespace if you want to _send_ events.
...
An event represents that ✨ something ✨ might have happened. An event is designated for a namespace and routing key and all matching listeners will
receive it. The routing key (topic) defines the type of the object of the event. For instance, an OS2mo object could have the type "`engagement`".

Events have a subject and a priority.

The default priority is `10000`, and most of the time there is no reason to change that. A priority of `1` is for the highest priority events, such as events requested by an end user action. Priorities make it possible to run large background synchronisations (e.g. an initial load of hundreds of thousands of engagements) without disturbing higher-priority traffic.

The subject is a stable identifier for the object that might have changed.
...
Events for native objects in OS2mo are always emitted in the public `mo` namespace.
...
`event_listener_declare` mutator is idempotent. A listener always belongs to the integration (actor) that declared it — you will never receive another owner's listener back.
...
These operations are not intended for programmatic use, but rather for developers to debug and maintain the system.
...
`event_unsilence` mutators, respectively. Silencing is typically used to
suppress known noise, e.g. alerts for events that fail while an underlying
...
and must handle them like any other event. Since silenced events are
continuously retried, they can resolve themselves once the underlying error
has been fixed — without requiring a manual `event_unsilence`.

You are only expected to silence events temporarily. Integrations MUST be able
to handle all events - it's fine to do nothing based on an event.

You should never acknowledge an event manually.

### Re-running events

The event_rerun mutator can be used to schedule events for immediate re-processing.
This is particularly useful when an integration configuration has been fixed to solve an operational problem, and you want to verify that the fix works — without having to unsilence events that are silenced for good reason.
...
Integrations SHOULD send an X-Request-ID header when calling MO. MO uses the same Request-ID mechanism as the integrations, which makes it easy to correlate logs in MO with the specific call — greatly simplifying debugging across the system landscape. The event-based AD integration already does this.
