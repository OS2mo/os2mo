...
* `owner` - which gives read/write permissions to everything for which your user is the owner. It is _necessary_ to give reader rights to owners as well, otherwise the owner role will not work properly.
...
Users in OS2mo contain information about their AD GUID. In order for all three roles (`admin`, `owner`, `reader`) to function properly, it is important to send the AD GUID of the logged in user as an attribute.
