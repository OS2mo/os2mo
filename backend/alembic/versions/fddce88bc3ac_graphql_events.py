...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("namespace_fk", sa.String, sa.ForeignKey("event_namespace.name"), nullable=False),
        sa.UniqueConstraint(
            "user_key",
            "owner",
            "namespace_fk",
    ...
    op.create_table(
        ...
        sa.Column("last_tried", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("'1970-01-01'::timestamptz")),
        ...
        sa.Column("listener_fk", sa.Uuid, sa.ForeignKey("event_listener.pk"), nullable=False),
        sa.UniqueConstraint(
