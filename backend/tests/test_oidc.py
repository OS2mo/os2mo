...
class TestOIDC(unittest.TestCase):
    def setUp(self) -> None:
        ...
        with open("backend/tests/mocking/mo/auth/jwtRS256.key.pub", "rb") as fp:
    ...
    def test_jwks_uri_correct(self):
        self.assertEqual(
            "http://keycloak:8080" "/auth/realms/mo/protocol/openid-connect/certs",
            oidc.JWKS_URI,
        )
    ...
    def test_auth_exception_handler_return_401_for_client_side_error(self):
        ...
        self.assertEqual(
            HTTP_401_UNAUTHORIZED, oidc.auth_exception_handler(None, exc).status_code
        )
    ...
    def test_auth_decodes_token(self, mock_get_signing_key_from_jwt):
        ...
        request = TestOIDC.generate_request_with_token(
            self.parsed_token, self.private_key
        )
    ...
    def test_raise_exception_for_invalid_signature(self, mock_get_signing_key_from_jwt):
        ...
        with open("backend/tests/mocking/mo/auth/hackers-jwtRS256.key", "rb") as fp:
            hackers_key = fp.read()

        # Create auth request with token signed with hackers key
        request = TestOIDC.generate_request_with_token(self.parsed_token, hackers_key)
...
class TestAuthError(unittest.TestCase):
    ...
    def test_is_client_side_error_false_for_http_exception_500(self):
        exc = AuthError(HTTPException(status_code=HTTP_500_INTERNAL_SERVER_ERROR))
