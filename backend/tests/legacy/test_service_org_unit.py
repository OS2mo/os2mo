...
class AsyncTestAddressLookup(tests.legacy.cases.AsyncTestCase):
...
class AsyncTestTriggerExternalIntegration(tests.legacy.cases.AsyncTestCase):
...
class AsyncTestGetOneOrgUnit(tests.legacy.cases.AsyncLoRATestCase):
...
class TestGetCountRelated(tests.legacy.cases.TestCase):
    def setUp(self):
        super().setUp()
...
class TestGetUnitAncestorTree(tests.legacy.cases.AsyncConfigTestCase):
    async def asyncSetUp(self):
        await super().asyncSetUp()
        self._connector = lora.Connector(
            virkningfra="-infinity", virkningtil="infinity"
