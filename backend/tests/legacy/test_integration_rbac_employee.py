...
from tests.legacy.test_integration_rbac import mock_auth
from tests.legacy.util import jsonfile_to_dict
from tests.legacy.util import override_config
...
class TestCommon(tests.legacy.cases.LoRATestCase):
    def setUp(self):
        super().setUp()
        self.create_owner_payload = jsonfile_to_dict(
            "legacy/fixtures/rbac/create_employee_owner.json"
        )
...
@pytest.mark.usefixtures("sample_structures")
class TestCreateEmployee(tests.legacy.cases.LoRATestCase):
    def setUp(self):
        super().setUp()
...
class TestCreateEmployeeDetailViaEmployee(TestCommon):
    ...
    def setUp(self):
        ...
        self.create_detail_payload = jsonfile_to_dict(
            "legacy/fixtures/rbac/create_employee_detail_phone.json"
        )
...
@pytest.mark.usefixtures("sample_structures")
class TestCreateEmployeeDetailViaOrgUnit(tests.legacy.cases.LoRATestCase):
    """
    ...
    def setUp(self) -> None:
        ...
        self.payload = jsonfile_to_dict(
            "legacy/fixtures/rbac/create_employee_detail.json"
        )
        self.payload[0]["person"]["uuid"] = LIS_JENSEN

        self.create_multiple_associations = jsonfile_to_dict(
            "legacy/fixtures/rbac/create_multiple_associations.json"
        )
    ...
    def test_create_multiple_employments_owns_one_unit_but_not_the_other(self):
        ...
        payload = jsonfile_to_dict(
            "legacy/fixtures/rbac/create_multiple_employments.json"
        )
...
class TestEditEmployeeDetail(TestCommon):
    def setUp(self) -> None:
        ...
        self.payload_edit_address = jsonfile_to_dict(
            "legacy/fixtures/rbac/edit_address.json"
        )

        self.payload_edit_employment = jsonfile_to_dict(
            "legacy/fixtures/rbac/edit_employment.json"
        )

        self.payload_edit_role = jsonfile_to_dict("legacy/fixtures/rbac/edit_role.json")

        self.payload_edit_association = jsonfile_to_dict(
            "legacy/fixtures/rbac/edit_association.json"
        )

        self.payload_edit_manager = jsonfile_to_dict(
            "legacy/fixtures/rbac/edit_manager.json"
        )
...
@pytest.mark.usefixtures("sample_structures")
class TestMoveEmployment(tests.legacy.cases.LoRATestCase):
    def setUp(self) -> None:
        super().setUp()
        # Move Erik Smidt Hansen from hum to samf
        self.move_single_employment_payload = jsonfile_to_dict(
            "legacy/fixtures/rbac/move_employment.json"
        )

        self.move_multiple_employments_payload = jsonfile_to_dict(
            "legacy/fixtures/rbac/move_multiple_employments.json"
        )
...
class TestEmployeeLeave(TestCommon):
    def setUp(self) -> None:
        super().setUp()
        self.payload_leave = jsonfile_to_dict("legacy/fixtures/rbac/leave.json")
