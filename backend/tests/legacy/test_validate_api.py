...
class TestValidateAPI(tests.legacy.cases.TestCase):
    ...
    def test_candidate_org_unit(self, mock):
        ...
        payload = {"org_unit": org_unit, "validity": {"from": from_date, "to": None}}

        self.client.post("/service/validate/org-unit/", json=payload)

        mock.assert_called_with(
            org_unit, mora_util.parsedatetime(from_date), mora_util.POSITIVE_INFINITY
        )
    ...
    def test_candidate_employee(self, mock):
        ...
        payload = {"person": person, "validity": {"from": from_date, "to": None}}

        self.client.post("/service/validate/employee/", json=payload)

        mock.assert_called_with(
            person, mora_util.parsedatetime(from_date), mora_util.POSITIVE_INFINITY
        )
    ...
    def test_existing_associations(self, mock):
        ...
        self.client.post("/service/validate/existing-associations/", json=payload)
