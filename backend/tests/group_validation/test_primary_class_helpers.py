...
def test_is_class_primary(mo_class: dict, expected_result: bool):
...
async def test_is_class_uuid_primary(
    primary_class_user_key: str, expected_result: bool
):
    with _mock_get_one_class(primary_class_user_key):
...
async def test_get_mo_object_primary_value(mo_object: dict, expected_result: bool):
    # Mock `get_one_class`, the return value is not important in this case
    with _mock_get_one_class(""):
        assert (await get_mo_object_primary_value(mo_object)) == expected_result


def _mock_get_one_class(primary_class_user_key: str):
    mock_get = mock.AsyncMock(return_value={mapping.USER_KEY: primary_class_user_key})
