...
class TestConfig(tests.cases.TestCase):
    ...
    def test_serviceplatformen_missing_path(self):
        with util.override_app_config(ENV="production"):
            with util.override_config({"dummy_mode": False, **self._sp_config()}):
        ...
        with tempfile.NamedTemporaryFile() as tf:
            with util.override_app_config(ENV="production"):
                with util.override_config(
                    {"dummy_mode": False, **self._sp_config(certificate_path=tf.name)}
                ):
                    with self.assertRaisesRegex(
    ...
    def test_serviceplatformen_invalid_values(self):
                ...
                with util.override_config(
                    {
                        "dummy_mode": False,
                        **self._sp_config(
                            system_uuid="some-other-string-with-4dashes", uuid="asd"
                        ),
                    }
                ):
                    with self.assertRaisesRegex(
                        ValueError,
                        "Serviceplatformen uuids must be valid: " "uuid, system_uuid",
                    ):
                        serviceplatformen.check_config()
        ...
        with util.override_app_config(ENV="production"):
            with util.override_config(
                ...
                with self.assertRaisesRegex(
                    FileNotFoundError, "Serviceplatformen certificate not found"
                ):
                    serviceplatformen.check_config()
