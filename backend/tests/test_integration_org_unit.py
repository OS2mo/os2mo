...
@pytest.mark.integration_test
@pytest.mark.usefixtures("fixture_db")
async def test_edit_org_unit_unset_parent_60582(graphapi_post):
    CREATE_ORG_UNIT = "mutation CreateOrgUnit($input: OrganisationUnitCreateInput!) { org_unit_create(input: $input) { uuid } }"
...
@pytest.mark.integration_test
@pytest.mark.usefixtures("fixture_db")
async def test_edit_org_unit_parent_history_60582(graphapi_post):
    CREATE_ORG_UNIT = "mutation CreateOrgUnit($input: OrganisationUnitCreateInput!) { org_unit_create(input: $input) { uuid } }"
    EDIT_ORG_UNIT = "mutation EditOrgUnit($input: OrganisationUnitUpdateInput!) { org_unit_update(input: $input) { uuid } }"
    ...
    for i in range(2):
        response = graphapi_post(
            ...
            {
                "input": {
                    "validity": {"from": "2020-01-01", "to": "2030-01-01"},
                    "name": f"Parent {i}",
                    "org_unit_type": "32547559-cfc1-4d97-94c6-70b192eff825",
                }
            },
        ...
        assert response.errors is None
        parents.append(response.data["org_unit_create"]["uuid"])
    ...
    response = graphapi_post(
        EDIT_ORG_UNIT,
        {
            "input": {
                "uuid": child_uuid,
                "validity": {"from": "2025-01-01", "to": "2030-01-01"},
                "name": "Child",
                "parent": parent1,
                "org_unit_type": "32547559-cfc1-4d97-94c6-70b192eff825",
            }
