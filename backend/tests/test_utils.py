...
@pytest.mark.parametrize("cpr, isodate, expected_raise", [
...
@pytest.mark.parametrize("value, path, val, expected", [
...
@pytest.mark.parametrize("value, expected, expected_raise", [
...
@pytest.mark.parametrize("key, default, required, expected_raise", [
    ...
    ("nonexistent", [], True,
     pytest.raises(exceptions.HTTPException,
                   match="ErrorCodes.V_MISSING_REQUIRED_VALUE")),
    ("nonexistent", {}, True,
     pytest.raises(exceptions.HTTPException,
                   match="ErrorCodes.V_MISSING_REQUIRED_VALUE")),
...
def test_checked_get_py(key, default, required, expected_raise):
    ...
    with expected_raise:
        assert util.checked_get(mapping, key, default, required=required) == \
