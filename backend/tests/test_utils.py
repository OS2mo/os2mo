...
# class TestUtilsPy:
@pytest.mark.parametrize("trying_time, expected_time", [
...
@pytest.mark.parametrize("trying_parse_date, expected_parse_date", [
...
@pytest.mark.parametrize("trying_uuid, expected", [
...
@pytest.mark.parametrize("trying_cpr, expected", [
    ("0101011000", True),
    ("123456789", False),
    ("42", False),
    (None, False)
...
@pytest.mark.parametrize("valid_cpr, isodate, expected_raise", [
...
@pytest.mark.parametrize("quote, expected_quote", [
    ("42", "42"),
    ("abc", "abc"),
    ("aBc", "a%42c"),
...
@pytest.mark.parametrize("obj, path, expected_location", [
    ({"whatever": "no", "test1": {"garbage": "there is some stuff here already",
                                  "test2": ["something"]}}, ("test1", "test2"),
     ["something"]),
...
@pytest.mark.parametrize("obj, path, value, expected_result", [
    ({"test1": {"test2": [{"key1": "val1"}]}}, ("test1", "test2"),
     [{"key2": "val2"}], {"test1": {"test2": [{"key1": "val1"}, {"key2": "val2"}]
                                    }}),  # Existing path.

    ({}, ("test1", "test2"), [{"key2": "val2"}],
     {"test1": {"test2": [{"key2": "val2"}]}}),  # New path.

    ({"test1": {"test2": "1337"}}, ("test1", "test2"), "42",
     {"test1": {"test2": "42"}}),  # Existing path string.
...
@pytest.mark.parametrize("valid_from, expected_result, expected_raise", [
    ...
    (({}, {"validity": {"from": None}}), None, pytest.raises(exceptions.HTTPException)),

    (({"validity": {"from": "2018-03-05"}}, None),  # From 477
     datetime.datetime(2018, 3, 5, tzinfo=util.DEFAULT_TIMEZONE), does_not_raise()),
...
@pytest.mark.parametrize("valid_to, expected_result", [
    ...
    (({"validity": {"to": "2018-03-05"}}, None),
     datetime.datetime(2018, 3, 6, tzinfo=util.DEFAULT_TIMEZONE)),  # 482
...
@pytest.mark.parametrize("validations, expected_result, expected_raise", [
    ({"validity": {"from": "2018-03-05", "to": "2018-04-04"}},
     ...
     does_not_raise()),  # From 550

    ({"validity": {"from": "2018-03-05"}},
     (datetime.datetime(2018, 3, 5, tzinfo=util.DEFAULT_TIMEZONE),
      util.POSITIVE_INFINITY), does_not_raise()),

    ({"validity": {"from": "2019-03-05", "to": "2018-03-05"}}, None,
     pytest.raises(exceptions.HTTPException)),
    ...
    ({"validity": {"from": "2018-03-05", "to": "2018-04-04"}},
     (datetime.datetime(2018, 3, 5, tzinfo=util.DEFAULT_TIMEZONE),  # 518
      datetime.datetime(2018, 4, 5, tzinfo=util.DEFAULT_TIMEZONE)), does_not_raise()),
    ...
    ({"description": "End date is before start date.",
      ...
      "error_key": "V_END_BEFORE_START",
      "obj": {"validity": {"from": "2019-03-05", "to": "2018-03-05"}},
      "status": 400}, None, pytest.raises(exceptions.HTTPException))
])
...
@pytest.mark.parametrize("value, expected_raise", [
...
@pytest.mark.parametrize("value", [
    ({"kaflaflibob": get_uuid_test_id(), "uuid": 42}, "kaflaflibob",)
])
