...
def test_namespace_delete(graphapi_post: GraphAPIPost) -> None:
    ...
    assert len(get_namespaces(graphapi_post)) == 2  # default mo namespace is there too
    delete_namespace(graphapi_post, ns)
    assert len(get_namespaces(graphapi_post)) == 1


@pytest.mark.integration_test
@pytest.mark.usefixtures("empty_db")
def test_mo_namespace_exists(graphapi_post: GraphAPIPost) -> None:
    namespaces = get_namespaces(graphapi_post)
    assert len(namespaces) == 1
    assert namespaces[0]["name"] == "mo"
