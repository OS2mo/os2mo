...
@pytest.mark.integration_test
@pytest.mark.usefixtures("load_fixture_data_with_reset")
async def test_refresh_itsystems(graphapi_post: Callable) -> None:
    """Test update_employee v12 validators."""
    test_input = {"filter": {}}

    # mutation under v14 schema
    mutation_v14 = """
        mutation TestITSystemFtiler($filter: BaseFilter!) {
            itsystem_refresh(filter: $filter)
        }
    """

    # mutation under v15 schema
    mutation_v15 = """
        mutation TestITSystemFtiler($filter: ITSystemFilter!) {
            itsystem_refresh(filter: $filter)
        }
    """
    ...
    expected = [
        ...
        "14466fb0-f9de-439c-a6c2-b3262c367da7",
        "59c135c9-2b15-41cc-97c8-b5dff7180beb",
    ]

    # Test it
    response_v14: GQLResponse = graphapi_post(
        mutation_v14, test_input, url="/graphql/v14"
    )
    assert response_v14.errors is None
    assert response_v14.data["itsystem_refresh"] == expected

    response_v15: GQLResponse = graphapi_post(
        mutation_v15, test_input, url="/graphql/v15"
    )
    assert response_v15.errors is None
    assert response_v15.data["itsystem_refresh"] == expected

    # Running v14 mutation on v15 schema
    response: GQLResponse = graphapi_post(mutation_v14, url="/graphql/v15")
    error = first(response.errors)
    assert (
        "Unknown type 'BaseFilter'. Did you mean 'ClassFilter', 'FacetFilter', "
        in error["message"]
    ...
    # Running v15 mutation on v14 schema
    response: GQLResponse = graphapi_post(mutation_v15, url="/graphql/v14")
    error = first(response.errors)
    assert (
        "Unknown type 'ITSystemFilter'. Did you mean 'ITUserFilter', 'BaseFilter', or 'ITSystem'?"
        in error["message"]
