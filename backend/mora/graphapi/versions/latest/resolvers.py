...
async def manager_resolver(
    ...
    if not org_unit:
        return []
    org_unit_list = list(flatten(org_unit.values()))
    parent_uuid = [org_unit.parent_uuid for org_unit in org_unit_list]
    return await manager_resolver(
        info, filter=ManagerFilter(org_units=parent_uuid), inherit=True
    )
