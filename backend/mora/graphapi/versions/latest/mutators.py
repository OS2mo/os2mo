...
async def refresh(
    ...
    if send_in_old_amqp:
        ...
        if exchange is not None:
            raise ValueError("owner and exchange are mutually exclusive. Exchanges are part of the legacy event system. If you are using GraphQL Events, do NOT use exchange.")
        session: AsyncSession = info.context["session"]
        for uuid in uuids:
            await add_event(
                session,
