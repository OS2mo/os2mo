...
# Post a user to MO
mo_employee_to_post = r.json()[-2]
new_nickname_givenname = "Man who can do %d push ups" % random.randint(0, 10_000)
mo_employee_to_post["nickname_givenname"] = new_nickname_givenname
...
assert r.json()["nickname_givenname"] == new_nickname_givenname
print("Successfully edited nickname_givenname to '%s' in MO" % new_nickname_givenname)


# Print the hyperlink to the employee.
print("")
url = "http://localhost:5000/medarbejder/%s#medarbejder" % mo_employee_to_post["uuid"]
print("see:\n%s\nto validate that the nickname was appropriately changed" % url)
