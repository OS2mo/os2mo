...
from mo_ldap_import_export.autogenerated_graphql_client.read_engagement_uuids import (
...
async def test_import_jobtitlefromadtomo_objects() -> None:
    ...
    graphql_client_mock.read_engagement_uuids.return_value = parse_obj_as(
        ReadEngagementUuidsEngagements,
        {
            "objects": [
                {
    ...
    graphql_client_mock.read_engagement_uuids.assert_awaited_once()
    call_args, call_kwargs = graphql_client_mock.read_engagement_uuids.await_args
    assert call_kwargs["engagement_filter"].employees == [test_user_uuid]
