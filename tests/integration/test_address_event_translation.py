...
async def test_address2orgunit(
    ...
    integration_graphql_client.org_unit_refresh = AsyncMock()  # type: ignore
    ...
    # Check that we send the event to MO
    integration_graphql_client.org_unit_refresh.assert_awaited_once_with(
...
async def test_address2orgunit_change_orgunit(
    ...
    # Check that we send the event to MO
    args = one(integration_graphql_client.org_unit_refresh.call_args_list).args
    exchange, uuids = args
    assert exchange == "os2mo_ldap_ie"
