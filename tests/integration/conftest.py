...
from mo_ldap_import_export.autogenerated_graphql_client import ClassFilter
...
@pytest.fixture
async def explicitly_primary(graphql_client: GraphQLClient) -> UUID:
    return one(
        (
            await graphql_client.read_class_uuid_by_facet_and_class_user_key(
                "primary_type", "explicitly-primary"
            )
        ).objects
    ).uuid
...
@pytest.fixture
async def mitid_uuid_address_type(graphql_client: GraphQLClient) -> UUID:
    # Check if the class already exists
    existing_classes = await graphql_client.read_class_uuid(
        filter=ClassFilter(user_keys=["MitIDUUID"])
    )
    if existing_classes.objects:
        # If multiple exist, just return the first one to avoid errors,
        # or better, return the one that matches our expectation if possible.
        # For now, just taking the first one is safer than failing.
        return existing_classes.objects[0].uuid

    # Find the employee_address_type facet
    facets = await graphql_client.read_facet_uuid(
        filter=FacetFilter(user_keys=["employee_address_type"])
    )
    facet_uuid = one(facets.objects).uuid

    # Create the MitIDUUID class
    r = await graphql_client.class_create(
        input=ClassCreateInput(
            ...
            scope="TEXT",
            validity=ValidityInput(**{"from": "1900-01-01T00:00:00Z"}),
        )
    )
    return r.uuid
