...
async def test_renaming_multi_valued_rdns(
    ...
    entry_uuid = await ldap_api.get_ldap_unique_ldap_uuid(person_dn)

    # Check that the entity got the expected DN
    assert await ldap_api.get_ldap_dn(entry_uuid) == person_dn
    ...
    new_dn = await ldap_api.get_ldap_dn(entry_uuid)
