...
async def test_filter_dns(
    ldap_api: LDAPAPI, ldap_person_dn: DN, filtered: bool
) -> None:
    settings = Settings()
    result = await filter_dns(
        settings, ldap_api.ldap_connection.connection, {ldap_person_dn}
    )
...
async def test_filter_dns_accepts_pre_moonlanding(
    ldap_api: LDAPAPI, add_ldap_person: AddLdapPerson
) -> None:
    ...
    settings = Settings()
    result = await filter_dns(
        settings, ldap_api.ldap_connection.connection, {b18, b19, a19, a20}
    )
