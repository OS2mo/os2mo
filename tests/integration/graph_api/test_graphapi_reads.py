    ...
    query = data.draw(
        gql_st.query(str(get_schema(LATEST_VERSION)), fields=[field]).filter(
            lambda query: (
                ...
                # We filter out any such bytes generated by hypothesis, in
                # either raw or `\uXXXX` escape form (the latter is what
                # `hypothesis-graphql` emits inside string literals).
