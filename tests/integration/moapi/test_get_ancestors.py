...
async def test_get_ancestors(
    ...
    ancestors = await mo_api.get_ancestors(uuid_map[child])
    assert [name_map[uuid] for uuid in ancestors] == expected


@pytest.mark.integration_test
@pytest.mark.usefixtures("test_client")
async def test_get_ancestors_unknown_uuid(mo_api: MOAPI) -> None:
    with pytest.raises(ValueError) as exc_info:
        await mo_api.get_ancestors(OrgUnitUUID(uuid4()))
