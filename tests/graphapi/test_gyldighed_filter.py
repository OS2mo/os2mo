...
async def test_employee_owner_filter_respects_gyldighed(
    ...
    """Filtering employees by owner must not match a person the owner role only
    points at while Inaktiv.

    Same misaligned shape as the engagement tests, but through the *reverse*
    ...
    person_b relation entirely within the Inaktiv period::
        ...
        gyldighed:  Aktiv [2020, 2021)    Inaktiv [2021, ...)
        person:     person_a [2020, 2023) person_b [2023, ...)
    """
    ...
    # Move the ownership to person_b from 2023. Terminating afterwards would
    # re-activate the owner role from the move date, so the move must happen
    # first.
    move = graphapi_post(
        """
    ...
    assert move.errors is None
    # Terminate in 2021, well before the move: person_b is now only ever owned
    # while the owner role is Inaktiv.
    terminate = graphapi_post(
        """
        ...
        """,
        variables={"input": {"uuid": str(owner), "to": "2021-01-01T00:00:00+01:00"}},
    )
    assert terminate.errors is None
    ...
    response = graphapi_post(
        ...
        variables={
            "filter": {
                "owner": {
                    "uuids": [str(owner)],
                    "from_date": "2020-01-01T00:00:00+01:00",
                    "to_date": None,
                },
                "from_date": "2020-01-01T00:00:00+01:00",
                "to_date": None,
    ...
    # Owned while Aktiv -> found; only owned while Inaktiv -> not found.
    assert employees == {person_a}


@pytest.mark.integration_test
async def test_org_unit_owner_filter_respects_gyldighed(
    empty_db: AsyncSession,
    create_org_unit: Callable[[str, UUID | None], UUID],
    create_person: Callable[..., UUID],
    create_owner: Callable[[dict[str, Any]], UUID],
    ...
    """Filtering org units by owner must not match a unit the owner role only
    points at while Inaktiv; the org-unit mirror of
    ``test_employee_owner_filter_respects_gyldighed``::
    ...
    response = graphapi_post(
        """
        query OrgUnits($filter: OrganisationUnitFilter!) {
    ...
    assert response.errors is None
    assert response.data is not None
    org_units = {UUID(o["uuid"]) for o in response.data["org_units"]["objects"]}
    # Owned while Aktiv -> found; only owned while Inaktiv -> not found.
    assert org_units == {unit_a}
