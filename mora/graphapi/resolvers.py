...
async def address_resolver_query(
    ...
    if (
        ...
        address_type_uuids = await filter2uuids_func(class_resolver, info, class_filter)
        query = query.where(
            OrganisationFunktionRegistrering.id.in_(
                select(
                    OrganisationFunktionRelation.organisationfunktion_registrering_id
    ...
    # Visibility. rel_type "opgaver" with objekt_type "synlighed" in mox.
    ...
    # Engagement / IT-user. Both filters share the `tilknyttedefunktioner` relation,
    # so when both are set we OR the resolved UUIDs into a single IN clause.
    if (
        filter.engagement is not None
        or filter.engagements is not None
        or filter.ituser is not None
    ):
        function_uuids: list[UUID] = []
        if filter.engagement is not None or filter.engagements is not None:
            function_uuids.extend(await get_engagement_uuids(info, filter))
        if filter.ituser is not None:
            function_uuids.extend(
        ...
        query = query.where(
            OrganisationFunktionRegistrering.id.in_(
                select(
                    ...
                    OrganisationFunktionRelation.rel_maal_uuid.in_(function_uuids),
                    _get_virkning_clause(OrganisationFunktionRelation, filter),
                )
            )
        )
