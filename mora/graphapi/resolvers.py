...
from mora.db import HasAktivVirkning
...
def _orgfunk_relation_loader(
    ...
    Field-level resolvers bind their filter to one root object each, so a query
    like `org_units(limit: 30) { engagements_response(filter: {...}) }` calls
    the engagement resolver 30 times with the same filter apart from the
    organisation unit. That organisation unit is the load key, and everything
    else picks the loader: one dataloader per distinct rest-of-filter, held for
    the duration of the request in `MOLoaders`.

    The batching lives here rather than in each field resolver because MO
    generates those fields through `seed_resolver`.
    """
...
async def _resolve_orgfunk_uuids(
    ...
    if limit is None and cursor is None:
        person_uuid = (
            ...
            if isinstance(filter, EmployeeFiltered)
            else None
        )
        ...
        org_unit_uuid = (
            _pinned_uuid(filter.org_unit, filter.org_units)
            if isinstance(filter, OrganisationUnitFiltered)
            else None
        )
        ...
        predicates.append(
            FacetRegistrering.id.in_(
                select(FacetAttrEgenskaber.facet_registrering_id).where(
                    ...
                    _get_active_period_clause(FacetAttrEgenskaber, filter),
                )
            )
        )
        ...
        predicates.append(
            FacetRegistrering.id.in_(
                select(FacetRelation.facet_registrering_id).where(
                    ...
                    _get_active_period_clause(FacetRelation, filter),
                )
            )
        )
        ...
        predicates.append(
            KlasseRegistrering.id.in_(
                select(KlasseAttrEgenskaber.klasse_registrering_id).where(
                    ...
                    _get_active_period_clause(KlasseAttrEgenskaber, filter),
                )
            )
        )
    ...
    def _funktionsnavn() -> ColumnElement:
        return OrganisationFunktionRegistrering.id.in_(
            select(
                ...
                _get_active_period_clause(OrganisationFunktionAttrEgenskaber, filter),
            )
        )
            ...
            OrganisationFunktionRegistrering.id.in_(
                select(
                    ...
                    _get_active_period_clause(
                        OrganisationFunktionAttrEgenskaber, filter
                    ),
                )
            )
        )
        ...
        predicates.append(
            BrugerRegistrering.id.in_(
                select(BrugerAttrEgenskaber.bruger_registrering_id).where(
                    ...
                    _get_active_period_clause(BrugerAttrEgenskaber, filter),
                )
            )
        )
        ...
        predicates.append(
            BrugerRegistrering.id.in_(
                select(BrugerRelation.bruger_registrering_id).where(
                    ...
                    _get_active_period_clause(BrugerRelation, filter),
                )
            )
        )
        ...
        predicates.append(
            OrganisationEnhedRegistrering.id.in_(
                select(
                    ...
                    _get_active_period_clause(OrganisationEnhedAttrEgenskaber, filter),
                )
            )
        )
        ...
        predicates.append(
            ITSystemRegistrering.id.in_(
                select(ITSystemAttrEgenskaber.itsystem_registrering_id).where(
                    ...
                    _get_active_period_clause(ITSystemAttrEgenskaber, filter),
                )
            )
        )
    ...
    if filter.external_ids is None:
        ...
        predicates.append(
            ~exists().where(
                ...
                _get_active_period_clause(OrganisationFunktionAttrUdvidelser, filter),
            )
        )
        ...
        predicates.append(
            OrganisationFunktionRegistrering.id.in_(
                select(
                    ...
                    _get_active_period_clause(
                        OrganisationFunktionAttrUdvidelser, filter
                    ),
                )
            )
        )
