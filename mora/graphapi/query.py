...
async def health_resolver(
    ...
    healths = list(healthchecks)[pagination.offset :]
    if pagination.limit is not None:
        healths = healths[: pagination.limit]
    return pagination.page(
        [
            Health(identifier=identifier)  # type: ignore[call-arg]
            for identifier in healths
        ],
        has_next_page=bool(healths),
    )
...
async def file_resolver(
    ...
    files = list(found_files)[pagination.offset :]
    if pagination.limit is not None:
        files = files[: pagination.limit]
