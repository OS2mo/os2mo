...
roles grants it on that object, wherever the object is reached. Whatever is
not guarded this way is still gated route by route in `mora.graphapi.rbac_map`.
...
OBJECTS_OF_COLLECTION: dict[
    Collection, Callable[[Sequence[UUID]], Select[tuple[UUID]]]
] = {}
...
async def policy_load_fn(
    ...
    for collection, uuids in uuids_by_collection(keys).items():
        ...
        for condition, fields in rules.get(collection, []):
            matching = objects(sorted(uuids)).where(condition).subquery()
            selects.append(
                select(
                    literal(collection),
