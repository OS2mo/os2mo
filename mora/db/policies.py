...
class PolicyRule(Base):
    ...
    # objects: a CEL expression returning one or more access-check specs, each
    # run as a SQL existence check against the entity being accessed (see
    # `_entity_filter_grants`). A null filter places no entity restriction.
    ...
    # policy. NULLs are treated as equal so a filter-less, unconditional rule is
    # also deduplicated (requires PostgreSQL >= 15).
    __table_args__ = (
        UniqueConstraint(
            "policy_fk",
            "type",
