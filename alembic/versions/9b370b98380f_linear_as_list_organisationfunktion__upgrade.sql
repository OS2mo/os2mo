...
-- Current (pre-rewrite) definitions of the eight as_list_<type> functions,
-- extracted from the database and formatted with pgFormatter.
--
-- In this commit the function bodies below are byte-identical to the matching
-- downgrade file, so the migration is a no-op reformat. The next commit
-- replaces the query bodies with the O(n) rewrite, which then shows up as a
-- clean per-function diff of this file.
--
-- See the commit message for exactly how the definitions were extracted and
-- formatted, so the result can be reproduced and validated.
-- ===================== organisation =====================
